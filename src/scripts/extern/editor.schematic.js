...
schematic = (function() {
	...
	function Schematic(input) {
	    ...
	    var toplevel = document.getElementById("editor-wrapper");
	...
	Schematic.prototype.save_netlist = function() {
		...
		console.log(convertNetList(localStorage.getItem("ckt")));
	};
...
function convertNetList(netList) {
	let newNetList = "",
	...
	// console.log(JSON.parse(netList)); // DEBUG

	// get x,y offsets to move circuit origin to 0,0
	for (const element of JSON.parse(netList)) {
		if (Array.isArray(element[1])) {
			offsetX = Math.min(offsetX, element[1][0]);
			offsetY = Math.min(offsetY, element[1][1]);
		}
	}
	
	if (offsetX == Number.MAX_SAFE_INTEGER) offsetX = 0;
	if (offsetY == Number.MAX_SAFE_INTEGER) offsetY = 0;

	// iterate each element of original notation of schematic
	for (const element of JSON.parse(netList)) {
		const elementCase = element[0].toLowerCase();
		switch (elementCase) {
			// condensator, inductor, resistor, source
			case "c":
			case "l":
			case "r":
			case "v":
				newNetList += `${elementCase.toUpperCase()}`;
				...
				switch (elementCase) {
					...
					case "v":
						...
						break;
					default:
						throw new Error(`Reached unreachable element case for: ${elementCase}`);
				}

				// set location
				newNetList += ` ${(element[1][0] - offsetX) / 8}`;
				newNetList += ` ${(element[1][1] - offsetY) / 8}`;

				// resistor value
				if (elementCase == "r")
					newNetList += ` {${element[2]["r"]}}`;

				// source values
				if (elementCase == "v") {
					// console.log(element); // DEBUG
					const sourceType = element[2]["value"].split("(")[0];
					switch (sourceType) {
						// sin for ac
						case "sin":
							newNetList += ` ac {${element[2]["value"].split("(")[1].split(")")[0].split(",")[1]}}`;
							newNetList += ` {2*pi*${element[2]["value"].split("(")[1].split(")")[0].split(",")[2]}}`;
							break;
						// skip dc
						case "dc":
							newNetList += ` dc {${element[2]["value"].split("(")[1].split(")")[0]}}`;
							break;
						// impulse, step, square, triangle, ramp, pwl, pwl (repeating), pulse
						default:
				...
				switch (element[1][2]) {
					...
					case 3:
						...
						break;
					default:
						throw new Error(`Unknown direction: ${JSON.stringify(element)}`);
				}
				newNetList += `\n`;
				break;
			// wires
			case "w":
				// abort if any wire runs not on grid
				if (element[1][0] != element[1][2] && element[1][1] != element[1][3])
					throw new Error(`Cross lines are not supported`);
				// wire up
				for (let i = element[1][0]; i < element[1][2]; i+=8)
					newNetList += `W ${(i - offsetX) / 8} ${(element[1][1] - offsetY) / 8}; right\n`;
				// wire down
				for (let i = element[1][0]; i > element[1][2]; i-=8)
					newNetList += `W ${(i - offsetX) / 8} ${(element[1][1] - offsetY) / 8}; left\n`;
				// wire left
				for (let i = element[1][1]; i < element[1][3]; i+=8)
					newNetList += `W ${(element[1][0] - offsetX) / 8} ${(i - offsetY) / 8}; down\n`;
				// wire right
				for (let i = element[1][1]; i > element[1][3]; i-=8)
					newNetList += `W ${(element[1][0] - offsetX) / 8} ${(i - offsetY) / 8}; up\n`;

				/*/ attempt cross lines WIP DEPRECATED
				let maxIteration = 1e9, posX = element[1][0], posY = element[1][1], i = 1;

				while (posX != element[1][2] || posY != element[1][3]) {

					if (posX < element[1][2]) newNetList += `W ${(posX++) - offsetX} ${posY - offsetY}; up\n`;
					else if (posX > element[1][2]) newNetList += `W ${(posX--) - offsetX} ${posY - offsetY}; down\n`;
					
					if (posY < element[1][3]) newNetList += `W ${posX - offsetX} ${(posY++) - offsetY}; right\n`;
					else if (posY > element[1][3]) newNetList += `W ${posX - offsetX} ${(posY--) - offsetY}; left\n`;

					if (i++ > maxIteration) throw new Error(`Aborting: Too many wires or infinite loop`);
				}/**/
				break;
			// skip these elements
			case "g": // WIP divergent behavior in editor and simplifier?
			case "view":
				break;
			// abort for unsuported elements
			default:
				throw new Error(`Unknown schematic type: ${JSON.stringify(element)}`);
		}
	}

	// WIP kabelsalat?
	// WIP detect separated ciruits
